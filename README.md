...
A real-time DeFi transaction processing and anomaly detection engine powered by **PySpark Structured Streaming**, **Apache Kafka**, a **Neo4j** graph database with **Graph Data Science (GDS)**, and an **Isolation Forest** ML anomaly signal — with a live **Grafana** dashboard and a standalone live graph view on top.
...
├── LICENSE
├── requirements.txt
├── .env.example              — copy to .env and set a real password
├── .gitignore
├── launch.ps1                — full pipeline bootstrap
├── docker-compose.yml        — Zookeeper, Kafka, Neo4j (+GDS), Grafana
│
├── pipeline/                 — all Python source, one flat folder
│   ├── graph_storage.py         Neo4j driver, schema, shared paths/constants
│   ├── transaction_generator.py Kafka producer (synthetic DeFi traffic)
│   ├── broadcast_engine.py      wallet-context broadcast join
│   ├── streaming_engine.py      Spark Structured Streaming + CEP + ML
│   ├── graph_analytics.py       GDS: PageRank, betweenness, Louvain, FastRP, wash-ring detection
│   ├── generate_wallet_profiles.py  synthetic wallet backstory generator
│   ├── load_wallet_profiles.py      loads that backstory into Neo4j
│   ├── train_isolation_forest.py    trains the anomaly-scoring model
│   ├── metrics_api.py               FastAPI service (Grafana + dashboard feed this)
│   ├── evaluate_model.py            precision/recall against ground truth
│   └── inspect_data.py              quick standalone Neo4j reader
│
├── dashboard/
│   └── graph_dashboard.html   — open directly in a browser, no server needed
│
├── data/
│   └── wallet_profiles.csv    — generated by generate_wallet_profiles.py
│
├── models/
│   ├── isoforest_model.joblib       — generated by train_isolation_forest.py
│   └── isoforest_model_meta.json
│
└── runtime/                   — generated at runtime; safe to delete for a clean slate
    ├── metrics_state.json         metrics_api.py's persisted state
    ├── eval_report.json           evaluate_model.py's last report
    └── checkpoints/                Structured Streaming checkpoint (neo4j_sink)
```

Every path above is computed in one place — `pipeline/graph_storage.py`'s `PROJECT_ROOT`, `MODELS_DIR`, `DATA_DIR`, and `RUNTIME_DIR` constants — so every other script imports those rather than recomputing its own relative path. If you ever reorganize further, that's the one file to update.

`pipeline/` is deliberately kept flat (no sub-packages) rather than nested further — every module in it imports its siblings with a plain `import graph_storage`-style statement, which only resolves correctly because they're run as scripts from that same directory (Python automatically puts a script's own folder on `sys.path`). This is why `launch.ps1` always invokes pipeline scripts by full path (`python pipeline\streaming_engine.py`) or, for `metrics_api.py` specifically (started via `uvicorn`'s module-string interface rather than a script path), `cd`s into `pipeline\` first — both patterns land the same imports.
...
- **Wash-trading ring detection could hang indefinitely** — the cycle-detection query enumerated every combination of parallel edges accumulated between the same wallets over a long-running session, which grows combinatorially. Now runs over a deduplicated edge first (see [Troubleshooting #14](#14-graph_analyticspy-hangs-indefinitely-at-detecting-wash-trading-rings)).
- **Everything lived at the project root** — reorganized into `pipeline/`, `dashboard/`, `data/`, `models/`, and `runtime/` (see [Project Structure](#project-structure)), with a single file (`graph_storage.py`) owning every cross-referenced path.
...
Brings up Zookeeper, Kafka (broker on `localhost:9092`), **Neo4j** (Bolt on `localhost:7687`, Browser UI on `http://localhost:7474`, credentials from your `.env`, with the GDS plugin pre-installed), Spark master/worker (optional, unused by default — the pipeline runs Spark locally via `local[*]`), and Grafana (`localhost:3000`, Infinity datasource pre-installed).
...
```powershell
python pipeline\generate_wallet_profiles.py
python pipeline\load_wallet_profiles.py
```

`load_wallet_profiles.py` also provisions the Neo4j schema (a uniqueness constraint on `Wallet.address` plus supporting indexes) — this replaces the original project's Hive equivalent (`provision_hive_database()` / `register_wallet_profiles_table()`, from before the Neo4j rearchitecture). Output lands in `data\wallet_profiles.csv`.

Optionally sanity-check the broadcast join before starting the stream:

```powershell
python pipeline\broadcast_engine.py
```
...
```powershell
python pipeline\train_isolation_forest.py
```

Saves the fitted model to `models\isoforest_model.joblib` and its metadata (feature list, contamination rate, decision threshold) to `models\isoforest_model_meta.json`. If this step is skipped, `streaming_engine.py` still runs — it logs a warning and falls back to rule-based detection only, and `ml_score`/`ml_anomaly` stay null/false.

(`launch.ps1` does this step for you automatically if `models\isoforest_model.joblib` doesn't already exist — it's only listed here for people following the manual steps instead. Training is deterministic (fixed random seed) and cheap, so there's no harm in re-running it; delete `models\isoforest_model.joblib` first if you want a fresh retrain.)

### Step 5: Run an Initial Graph Analytics Pass

```powershell
python pipeline\graph_analytics.py
```

Projects the transaction graph in Neo4j GDS and runs PageRank, betweenness centrality, Louvain community detection, FastRP node embeddings, and wash-trading ring (cycle) detection, writing the results onto `Wallet` nodes. Run this once before starting the stream so the very first broadcast join has real `graph_risk_score`/`structural_novelty_score` values instead of all-zero defaults, and **re-run it periodically** while the pipeline is running — see [Keeping Graph Analytics Fresh](#keeping-graph-analytics-fresh).
...
The `cd pipeline` first matters here specifically: `uvicorn`'s `module:app` syntax resolves `metrics_api` by Python's normal import search, which depends on the working directory, not the script's own location — running this from the project root instead (even with a `pipeline.metrics_api:app` module path) breaks `metrics_api.py`'s own `from graph_storage import ...` line, since `graph_storage.py` is a sibling file expecting to be found via the working directory. Every other script in this project (invoked as `python pipeline\script.py`) doesn't have this issue, since Python automatically adds a script's own folder to its import path when you run it that way — `uvicorn` is the one exception, since it imports by module name rather than running a file.

Includes three new endpoints beyond the original `/metrics/summary` (see [New Metrics API Endpoints](#new-metrics-api-endpoints)).

### Step 7: Run the Transaction Producer

```powershell
python pipeline\transaction_generator.py
```
...
```powershell
python pipeline\streaming_engine.py
```

### Step 9: Watch It Live, Then Check the Numbers

Open `dashboard\graph_dashboard.html` directly in a browser for a live force-directed view. After a few minutes of traffic, run:
...
`graph_analytics.py` is a periodic batch job, not a streaming one — GDS graph projection isn't designed to run at micro-batch latency. For a live demo, run it on a timer alongside the pipeline, e.g. in its own PowerShell window (note the `cd` — see the note in Step 6 above about why pipeline scripts need to be run from inside `pipeline\`, or invoked by full path):
...
against the label `normal` vs. everything else. It also writes the full report to `runtime\eval_report.json`. Run it after letting `transaction_generator.py` + `streaming_engine.py` run for a few minutes — at this project's ~2% anomaly rate you want at least a few hundred transactions before the precision/recall numbers mean anything. If a number looks bad, that's the CEP threshold constants in `graph_storage.py` or the Isolation Forest's `CONTAMINATION` in `train_isolation_forest.py` telling you where to look next, not a sign something is broken.
...
* **Cause:** no `.env` file exists yet at the project root, or the variable name inside it is misspelled.
* **Fix:** `Copy-Item .env.example .env` at the project root (not inside `pipeline\`), then edit `.env` and set a real `NEO4J_PASSWORD`. `graph_storage.py` loads `.env` from an explicit path next to itself one level up (`PROJECT_ROOT/.env`), so this works regardless of which directory you run a script from — the file just needs to actually exist at the project root.

### 5. `streaming_engine.py` logs "Could not load Isolation Forest model"

* **Cause:** `train_isolation_forest.py` hasn't been run yet (or `models\isoforest_model.joblib` was deleted).
* **Fix:** run `python pipeline\train_isolation_forest.py` once. The pipeline still runs without it — it just falls back to rule-based CEP detection only.
...
* **Fix:** current `metrics_api.py` persists to `runtime\metrics_state.json` and reloads on startup. If totals are still resetting, check that the process has write permission to that directory, and look for a `[WARN] Failed to persist metrics state` line in its console output.
...
### 14. `graph_analytics.py` hangs indefinitely at "Detecting wash-trading rings"

* **Cause:** `write_transactions_batch()` intentionally creates a new `SENT` relationship per `(sender, receiver, window_start)` for idempotency, so the same small ring of wallets accumulates more and more *parallel* edges the longer the pipeline runs. Cycle detection has to enumerate every combination of parallel edges along a path — with even a few dozen parallel edges per hop, a 3–8 hop search blows up combinatorially. This is the query being asked to do exponentially more work than necessary, not a slow machine.
* **Fix:** already applied in `detect_wash_rings()` — it collapses every wallet pair down to a single deduplicated `:TRANSACTED_WITH` edge before searching for cycles, turning the search back into what it should be: a fast search over a small simple graph, regardless of how much raw transaction history has piled up. A 30-second query timeout is also in place as a safety net (`CYCLE_QUERY_TIMEOUT_SECONDS` in `graph_analytics.py`) so a future edge case can't hang the periodic refresh loop indefinitely — a timeout there is logged as a warning and the run continues with whatever rings were already found, rather than losing that run's PageRank/betweenness/Louvain/FastRP results too.
